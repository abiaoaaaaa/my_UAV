...
class DroneEnv(gym.Env):
    def __init__(self):
        ...
        # 定义状态空间和动作空间
        #状态空间x,y,v,航向角
        self.observation_space = gym.spaces.Box(low=np.array([0,0]), high=np.array([35000,20000]),dtype=np.float32)
        #动作空间加速度、角速度
        self.action_space = gym.spaces.Discrete(4)

        # 定义起点、终点和障碍物
        self.start = (5000,10000)  # 初始状态：x, y, 速度，角度
        self.goal = (30000, 13000)
        self.obstacles = [8000, 10700]

        # 初始化无人机状态
        self.drone_state =  [float(x) for x in np.array(self.start)]
        # 定义奖励函数中的常数
        self.goal_reward = 10000
        self.collision_penalty = -5000
        self.Border_penalty = -10000
        self.step_penalty = -1
        self.t_step = 1 #时间步
        self.r_obstacles = 500#障碍半径
        self.r_goal = 500 #目标半径
        self.done1 = False
        self.change = [[0, -200], [0, 200], [-200, 0], [200, 0]]#下、上、左、右
    ...
    def step(self, action):
        ...
        if (abs(self.drone_state[0] - self.goal[0]) < self.r_goal) and  (abs(self.drone_state[1] - self.goal[1]) < self.r_goal):
            ...
            self.done1 = True
        # 判断是否障碍
        if (abs(self.drone_state[0] - self.obstacles[0]) < self.r_goal) and  (abs(self.drone_state[1] - self.obstacles[1]) < self.r_goal):
            #print("111")
            self.done1 = True
