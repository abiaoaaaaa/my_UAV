...
class DroneEnv(gym.Env):
    def __init__(self):
        ...
        # 定义状态空间和动作空间
        #状态空间x,y,v,航向角
        self.observation_space = gym.spaces.Box(low=np.array([0,0,0]), high=np.array([35000,20000, 2*np.pi*10000]),dtype=np.float32)
        #动作空间加速度、角速度
        self.action_space = gym.spaces.Discrete(3)

       # 定义起点、终点和障碍物
        self.start = [5000, 10000,0]  # 初始状态：x, y, 速度，角度
        self.goal = [30000, 13000,0]
        #self.obstacles = [(10, 90)]
        self.drone_state =  [0,0,0]
        # 初始化无人机状态
        for i in range(3):
            self.drone_state[i] = self.start[i]
        # 定义奖励函数中的常数
        self.goal_reward = 10000
        self.collision_penalty = -50
        self.Border_penalty = -10000
        self.step_penalty = -1
        self.t_step = 1 #时间步
        self.r_obstacles = 5#障碍半径
        self.r_goal = 500 #目标半径
        self.done1 = False
        self.jiaodu = 0
        self.change = [-300,0,300]#角速度
    ...
    def step(self, action):
        ...
        # 更新状态
        self.drone_state[2] += self.change[action] * self.t_step# 更新角度
        self.drone_state[0] += (250 * self.t_step) * math.cos( self.drone_state[2]*0.0001) # 更新x坐标
        self.drone_state[1] += (250 * self.t_step) * math.sin( self.drone_state[2]*0.0001) # 更新y坐标
    ...
    def calculate_reward(self,prev_state): # 计算奖励
        ...
        if (self.drone_state[0] < self.observation_space.low[0] or self.drone_state[0] > self.observation_space.high[0] or self.drone_state[1] < self.observation_space.low[1] or self.drone_state[1] > self.observation_space.high[1]):
            ...
            return d_lod**0.5 - d_new**0.5 -130
