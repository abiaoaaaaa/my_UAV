...
class DroneEnv(gym.Env):
    ...
    def step(self, action):
        ...
        action_p, _states = self.model_p.predict(self.observation, deterministic=True)
        # 执行P动作
    ...
    def determine(self):
        ...
        if (self.get_d2goal() < self.r_goal):
            self.done1 = True
         #判断是否超出边界
        if (self.xy_p[0] < self.space1.low[0] or self.xy_p[0] > self.space1.high[0] or self.xy_e[1] < self.space1.low[1] or self.xy_e[1] > self.space1.high[1]):
            #print("222")
            self.done1 = True

    def _human_render(self):
        # Implement human-readable rendering logic here
        plt.scatter(self.xy_p[0], self.xy_p[1], marker='o', color='red', label='Drone')
        plt.scatter(self.xy_e[0], self.xy_e[1], marker='o', color='g', label='Drone')
        #plt.show()
