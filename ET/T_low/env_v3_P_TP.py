...
class DroneEnv(gym.Env):
    def __init__(self):
        ...
        self.xy_e_next = [0, 0]
        self.id = 0
        self.data_pre = np.array([[0 for _ in range(2)] for _ in range(10)], dtype='float64')
        self.data_true = np.array([[0 for _ in range(2)] for _ in range(10)], dtype='float64')
    ...
    def step(self, action):
        ...
        # 更新状态
        self.drone_state_p[0] = self.get_angle2pre_goal()
    ...
    def reset(self,seed=None, options=None):
        ...
        #初始化预测窗口和真实窗口
        for i in range(10):
            self.data_pre = np.insert(self.data_pre, self.data_pre.shape[0], self.xy_e, axis=0)[1:]
            self.data_true = np.insert(self.data_true, self.data_true.shape[0], self.xy_e, axis=0)[1:]
        self.xy_e_next = self.model_TP(self.data_pre.reshape(1, 10, 2) / 2000)
        self.xy_e_next = self.xy_e_next[0]
        self.xy_e_next = self.xy_e_next * 2000
